...
const App = () => {
  ...
  return (
    <div className="min-h-screen text-gray-900 font-sans flex flex-col overflow-x-hidden relative">
      <BackgroundImage />
      
      <header className="bg-white/60 backdrop-blur-md border-b border-white/40 sticky top-0 z-40 w-full shadow-sm">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 overflow-y-visible relative z-10">
        {renderContent()}
      </main>
    </div>
  );
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      <div className="flex-1 bg-white/60 backdrop-blur-md rounded-2xl shadow-xl border border-white/40 flex flex-col h-fit">
        <div className="p-4 border-b border-white/40 bg-white/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 rounded-t-2xl">
          <select 
          ...
          {activePassage.content.map((paragraph: string, pIdx: number) => (
            <p key={pIdx} className="mb-8 last:mb-0">
              {splitParagraphIntoWords(paragraph).map((item) => (
                <span 
                  key={item.id} 
                  className={item.isWord ? "cursor-pointer hover:bg-indigo-200/50 hover:text-indigo-900 hover:shadow-[0_0_0_2px_rgba(99,102,241,0.2)] rounded px-0.5 transition-all duration-150 active:scale-95 inline-block" : ""} 
...
const QuestionsPanel = ({ passage }: { passage: Passage }) => {
  return (
    <div className="bg-white/70 backdrop-blur-md rounded-2xl shadow-xl border border-white/40 overflow-hidden flex flex-col lg:max-h-[calc(100vh-140px)]">
      <div className="p-5 border-b border-white/40 bg-white/40 font-bold text-gray-900 flex items-center gap-3">
        ...
        {passage.questions.map((q) => (
          <div key={q.id} className="space-y-4">
            <div className="flex gap-4">
              ...
              <p className="text-gray-900 text-sm font-bold leading-snug">{q.text}</p>
            </div>
            <div className="ml-11">
              {q.type === 'TRUE_FALSE_NOT_GIVEN' || q.type === 'YES_NO_NOT_GIVEN' ? (
                <div className="flex flex-wrap gap-4">
                  {(q.type === 'TRUE_FALSE_NOT_GIVEN' ? ['TRUE', 'FALSE', 'NOT GIVEN'] : ['YES', 'NO', 'NOT GIVEN']).map(opt => (
                    <label key={opt} className="flex items-center gap-2 cursor-pointer text-[11px] font-black text-gray-600 hover:text-indigo-700 transition-colors">
                ...
                <div className="space-y-2">
                  {q.options?.map((opt, idx) => (
                    <label key={idx} className="flex items-start gap-3 cursor-pointer text-sm text-gray-700 hover:text-gray-900 transition-colors p-2 rounded-lg hover:bg-white/40 group">
                ...
                <input 
                  ...
                  className="w-full border border-white/60 rounded-xl text-sm p-3 focus:ring-2 focus:ring-indigo-50 outline-none bg-white/40 placeholder:text-gray-500" 
                />
              )}
            </div>
          </div>
...
const TranslationModal = ({ word, context, onClose, onSave }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div 
        className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl w-full max-w-md p-8 relative animate-in zoom-in-95 border border-white/60"
        onClick={e => e.stopPropagation()}
      >
        <button onClick={onClose} className="absolute top-8 right-8 text-gray-500 hover:text-gray-900 p-2 hover:bg-white/50 rounded-full transition-colors">
          <X size={20} />
        </button>
        ...
        <div className="space-y-8">
          <div className="p-6 bg-indigo-50/40 rounded-3xl border border-indigo-100/50 shadow-sm">
            <label className="text-[10px] font-black text-indigo-500 uppercase tracking-widest block mb-3">Russian Translation</label>
            <p className="text-3xl text-indigo-800 font-black leading-none">{data.translation}</p>
          </div>
          
          <div className="space-y-6">
            <div>
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2">Academic Meaning</label>
              <p className="text-gray-800 font-medium leading-relaxed">{data.definition}</p>
            </div>

            {data.synonyms && data.synonyms.length > 0 && (
              <div>
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-3">Synonyms</label>
                <div className="flex flex-wrap gap-2">
                  {data.synonyms.map((s: string, idx: number) => (
                    <span key={idx} className="px-3 py-1.5 bg-white/60 text-gray-700 rounded-xl text-xs font-black capitalize border border-white/60">
                      {s}
                    </span>
                  ))}
                </div>
...
const QuizView = ({ passages }: { passages: Passage[] }) => {
  ...
  const generateQuiz = async () => {
    ...
    // Dynamic Quiz Generation Strategy - Ensure 15 questions every time
    // 1. Extract valid dictionary words from the text
    const candidateWords = new Set<string>();
    
    passage.content.forEach(para => {
       const tokens = para.split(/[\s,."():;!?]+/);
       tokens.forEach(raw => {
          const w = raw.toLowerCase().replace(/['’]s$/, '').replace(/^[^\wА-Яа-я]+|[^\wА-Яа-я]+$/g, '');
          if (w.length > 3 && LOCAL_DICTIONARY[w]) {
             candidateWords.add(w);
          }
       });
    });

    // 2. Select 15 random words
    let deck = Array.from(candidateWords);
    if (deck.length < 15) {
        const allDictKeys = Object.keys(LOCAL_DICTIONARY).filter(k => k.length > 3);
        while (deck.length < 15) {
            const randomWord = allDictKeys[Math.floor(Math.random() * allDictKeys.length)];
            if (!deck.includes(randomWord)) {
                deck.push(randomWord);
            }
        }
    }
    const selectedWords = deck.sort(() => 0.5 - Math.random()).slice(0, 15);
    
    // 3. Construct Questions
    const newQuestions: QuizItem[] = selectedWords.map(word => {
       const entry = LOCAL_DICTIONARY[word];
       
       const distractors = new Set<string>();
       const allKeys = Object.keys(LOCAL_DICTIONARY);
       let attempts = 0;
       while(distractors.size < 3 && attempts < 50) {
          const rand = allKeys[Math.floor(Math.random() * allKeys.length)];
          const distDef = LOCAL_DICTIONARY[rand]?.d;
          if (distDef && distDef !== entry.d && !distractors.has(distDef)) {
             distractors.add(distDef);
          }
          attempts++;
       }
       
       const options = Array.from(distractors);
       options.push(entry.d);
       options.sort(() => 0.5 - Math.random());
       
       let contextSentence = "Found in text.";
       for (const para of passage.content) {
          const sentences = para.match(/[^.!?]+[.!?]+/g) || [para];
          const match = sentences.find(s => {
              const cleanS = s.toLowerCase();
              return cleanS.includes(` ${word} `) || cleanS.startsWith(`${word} `) || cleanS.includes(` ${word}.`);
          });
          if (match) {
       ...
       return {
          ...
          options: options,
          correctAnswerIndex: options.indexOf(entry.d),
          explanation: `Translation: ${entry.t}. \nContext: "${contextSentence}"`
       };
    });
  ...
  if (stage === 'SETUP') return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      <div className="bg-white/60 backdrop-blur-md p-10 md:p-16 rounded-[2.5rem] shadow-xl space-y-10 text-center border border-white/40 flex flex-col items-center">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-600 font-medium text-lg max-w-sm mx-auto">Master high-level academic vocabulary from the official reading texts.</p>
        </div>
        <div className="w-full max-w-sm text-left space-y-3">
          <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-1">Source Reading Text</label>
          <select 
            value={selectedPassageId} 
            onChange={(e) => setSelectedPassageId(Number(e.target.value))} 
            className="w-full p-5 border border-white/60 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none bg-white/40 font-bold text-gray-700 transition-all cursor-pointer backdrop-blur-sm"
          >
  ...
  if (stage === 'LOADING') return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 text-center py-20">
      <div className="bg-white/50 backdrop-blur-md p-8 rounded-3xl shadow-lg">
        <Loader2 className="w-20 h-20 animate-spin text-indigo-700 mx-auto mb-4" />
        <h3 className="text-3xl font-black tracking-tight text-gray-900">Preparing 15 Questions...</h3>
      </div>
    </div>
  );
  ...
  if (stage === 'RESULT') return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      <div className="bg-white/60 backdrop-blur-md p-12 md:p-16 rounded-[3rem] shadow-2xl text-center border border-white/40">
        <Trophy className="w-24 h-24 text-yellow-500 mx-auto mb-8" />
        <h2 className="text-7xl font-black mb-4 text-gray-900 leading-none">{score} <span className="text-3xl text-gray-500 font-bold">/ {questions.length}</span></h2>
      ...
      <div className="mb-10 space-y-5 px-2">
        <div className="flex justify-between items-end">
          <div className="space-y-1">
             <span className="px-3 py-1 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-sm">
               Question {currentQIndex + 1}
             </span>
             <h2 className="text-3xl font-black text-white tracking-tight drop-shadow-md">Define the Word</h2>
          </div>
          <div className="px-5 py-2 bg-green-500 text-white rounded-2xl text-xl font-black shadow-lg border border-green-400">
             {score}
          </div>
        </div>
        <div className="w-full h-4 bg-white/30 rounded-full overflow-hidden p-1 shadow-inner backdrop-blur-sm border border-white/20">
          <div className="h-full bg-indigo-500 transition-all duration-700 ease-out rounded-full" style={{ width: `${progress}%` }}></div>
        </div>
      </div>

      <div className="bg-white/70 backdrop-blur-md rounded-[3rem] shadow-2xl border border-white/40 overflow-hidden">
        <div className="p-12 bg-indigo-900/90 text-white relative">
        ...
        <div className="p-10 md:p-14 grid grid-cols-1 md:grid-cols-2 gap-5">
          {curQ.options.map((opt, idx) => (
            <button key={idx} onClick={() => handleSelectAnswer(idx)} disabled={isAnswered} className={`w-full text-left p-6 rounded-3xl border-2 font-bold transition-all flex items-center gap-5 ${isAnswered ? (idx === curQ.correctAnswerIndex ? 'bg-green-100/80 border-green-500 text-green-800' : (idx === selectedOption ? 'bg-red-100/80 border-red-500 text-red-800' : 'opacity-40')) : 'border-white/50 bg-white/40 hover:border-indigo-400 hover:bg-white/60 active:scale-95'}`}>
              <span className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black border-2 ${isAnswered && idx === curQ.correctAnswerIndex ? 'bg-green-500 text-white border-green-500' : 'border-gray-200 text-gray-400'}`}>{String.fromCharCode(65 + idx)}</span>
              <span className="flex-1 text-lg leading-tight text-gray-800">{opt}</span>
            </button>
          ))}
        </div>

        {isAnswered && (
          <div className="px-10 md:px-14 py-10 bg-white/40 border-t border-white/40">
             <p className="text-gray-800 text-lg leading-relaxed font-medium"><strong>Context:</strong> {curQ.explanation}</p>
             <button onClick={nextQuestion} className="mt-8 w-full py-4 bg-gray-900 text-white rounded-2xl font-black text-lg shadow-lg">Next Question</button>
          </div>
        )}
      </div>
    </div>
...
const VocabularyView = ({ words, onUpdateProgress, onDeleteWord }: any) => {
  ...
  if (isQuizMode) {
    if (currentIdx >= quizWords.length) return (
      <div className="max-w-2xl mx-auto py-20 px-4 text-center bg-white/60 backdrop-blur-md rounded-[3rem] shadow-2xl flex flex-col items-center border border-white/40">
    ...
    return (
      <div className="max-w-xl mx-auto py-12 px-4 flex flex-col gap-10">
        <div className="aspect-[4/3] bg-white/60 backdrop-blur-md rounded-[3.5rem] shadow-2xl flex flex-col items-center justify-center p-12 cursor-pointer border-4 border-white/40 hover:border-indigo-200 transition-all" onClick={() => setShowBack(!showBack)}>
          ...
          <p className="mt-12 text-indigo-700 font-black uppercase text-[10px] tracking-widest">{showBack ? card.definition : 'Tap to Flip'}</p>
        </div>
        {showBack && (
          <div className="flex gap-6">
            <button onClick={() => { onUpdateProgress(card.id, false); setCurrentIdx(i => i + 1); setShowBack(false); }} className="flex-1 py-8 bg-red-100/80 backdrop-blur-sm text-red-700 rounded-3xl font-black shadow-lg border border-red-200">FORGOT</button>
            <button onClick={() => { onUpdateProgress(card.id, true); setCurrentIdx(i => i + 1); setShowBack(false); }} className="flex-1 py-8 bg-green-100/80 backdrop-blur-sm text-green-700 rounded-3xl font-black shadow-lg border border-green-200">EASY</button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="space-y-12">
      <div className="bg-indigo-900/80 backdrop-blur-md p-12 md:p-16 rounded-[4rem] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden border border-white/20">
        ...
        <button onClick={() => { setQuizWords([...words].sort(() => 0.5 - Math.random())); setIsQuizMode(true); }} disabled={words.length === 0} className="relative z-10 px-12 py-6 bg-white/90 text-indigo-800 rounded-3xl font-black text-xl shadow-2xl disabled:opacity-50">START REVIEW</button>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
        {words.length === 0 ? (
          <div className="col-span-full py-40 text-center bg-white/40 backdrop-blur-md rounded-[4rem] border-4 border-dashed border-gray-300 text-gray-500 text-2xl font-black">Your Dictionary is Empty</div>
        ) : (
          words.map((w: any) => (
            <div key={w.id} className="bg-white/60 backdrop-blur-sm p-10 rounded-[2.5rem] border border-white/40 shadow-sm relative group hover:shadow-2xl transition-all flex flex-col h-full hover:bg-white/70">
              <div className="flex justify-between items-start mb-6">
                <h4 className="font-black text-3xl text-gray-900 capitalize leading-none tracking-tighter">{w.word}</h4>
                <div className="flex gap-2">
                   <button onClick={() => onDeleteWord(w.id)} className="text-gray-400 group-hover:text-red-500 transition-all p-2"><X size={18} /></button>
                </div>
              </div>
              <div className="p-5 bg-indigo-50/40 rounded-2xl border border-indigo-100/50 mb-6 mt-auto">
                 <p className="text-indigo-900 font-black text-2xl tracking-tight">{w.translation}</p>
              </div>
              <div className="space-y-3">
                <p className="text-xs text-gray-600 font-bold italic line-clamp-2">"{w.definition}"</p>
                {w.synonyms && w.synonyms.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {w.synonyms.slice(0, 3).map((s: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 bg-white/50 text-[9px] font-black text-gray-500 border border-white/50 rounded-md">
                        {s}
                      </span>
                    ))}
                  </div>
